...
$env:EMBEDDING_URL = "http://localhost:11500"
$env:EMBEDDING_MODEL = "nomic-embed-text"
$env:MCP_SERVER_PROJECT = (Resolve-Path .\SupportMcpServer\SupportMcpServer.csproj)
dotnet run --project AgentTools.Api
```

Compose starts Qdrant and Ollama, then pulls `nomic-embed-text` automatically through the one-shot `ollama-model-init` service. When `QDRANT_URL` is not set, the MCP server uses an in-memory fallback. When Qdrant is enabled, Ollama and the model must be running because the sample uses real 768-dimensional semantic embeddings.
...
Unit tests cover deterministic order and knowledge lookup. Integration tests host the real API, MCP server, DeepSeek agent, and Qdrant retrieval path; they automatically require `DS_KEY`.
